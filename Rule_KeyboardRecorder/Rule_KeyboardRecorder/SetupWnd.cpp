...
BOOL CSetupWnd::InitWnd(CWnd* pParent)
{
	...
	else
	{
		...
		m_btnGroup->Create(strText, WS_CHILD | WS_VISIBLE | BS_GROUPBOX, CRect(0, 0, rect.right - 20, 60), this, IDC_BTN01);
	}
	...
	if (m_stPath)
	{
		...
		m_stPath->Create(strText, WS_CHILD | WS_VISIBLE, CRect(200, 15, 500, 30), this, IDC_ST02);
	}

	m_edPath = new CEdit();
	if (m_edPath)
	{
		m_edPath->Create(WS_CHILD | WS_VISIBLE | WS_TABSTOP | WS_BORDER | ES_AUTOHSCROLL, CRect(200, 33, 470, 53), this, IDC_ED02);
