...
#include <memory.h>
#include <mmsystem.h>
#include <vfw.h>	// c:\Program Files (x86)\Windows Kits\8.1\Include\um\Vfw.h
...
BOOL AVI_FileOpenWrite(PAVIFILE * pfile, CString strName)
{
...
{	
	if(value.CompareNoCase(L"DIB") == 0)
	{
		return mmioFOURCC(value[0],value[1],value[2],' ');
	}
	else if((value.CompareNoCase(L"CVID") == 0)
		 || (value.CompareNoCase(L"IV32") == 0)
		 || (value.CompareNoCase(L"MSVC") == 0)
		 || (value.CompareNoCase(L"IV50") == 0))
	{
...
{
	CString tmp = L"";
	CString returnValue = L"Dump of AVICOMPRESSOPTIONS\n";

	tmp.Format(L"DWORD  fccType = streamtype(%s)\n", getFOURCCVAsString(opts.fccType));
	returnValue += tmp;
 
	tmp.Format(L"DWORD  fccHandler = %s\n", getFOURCCVAsString(opts.fccHandler));
	returnValue += tmp;

	tmp.Format(L"DWORD  dwKeyFrameEvery = %d\n", opts.dwKeyFrameEvery);
	returnValue += tmp;

	tmp.Format(L"DWORD  dwQuality = %d\n", opts.dwQuality);
	returnValue += tmp;

	tmp.Format(L"DWORD  dwBytesPerSecond = %d\n", opts.dwBytesPerSecond);
	returnValue += tmp;

	if(opts.dwFlags & (AVICOMPRESSF_DATARATE == AVICOMPRESSF_DATARATE)){tmp.Format(L"DWORD  fccType = AVICOMPRESSF_DATARATE\n");}
	else if(opts.dwFlags & (AVICOMPRESSF_INTERLEAVE == AVICOMPRESSF_INTERLEAVE)){tmp.Format(L"DWORD  fccType = AVICOMPRESSF_INTERLEAVE\n");}
	else if(opts.dwFlags & (AVICOMPRESSF_KEYFRAMES == AVICOMPRESSF_KEYFRAMES)){tmp.Format(L"DWORD  fccType = AVICOMPRESSF_KEYFRAMES\n");}
	else if(opts.dwFlags & (AVICOMPRESSF_VALID == AVICOMPRESSF_VALID)){tmp.Format(L"DWORD  fccType = AVICOMPRESSF_VALID\n");}
	else {tmp.Format(L"DWORD  dwFlags = Unknown(%d)\n", opts.dwFlags);}
	returnValue += tmp;

	tmp.Format(L"LPVOID lpFormat = %d\n", opts.lpFormat);
	returnValue += tmp;

	tmp.Format(L"DWORD  cbFormat = %d\n", opts.cbFormat);
	returnValue += tmp;

	tmp.Format(L"LPVOID lpParms = %d\n", opts.lpParms);
	returnValue += tmp;

	tmp.Format(L"DWORD  cbParms = %d\n", opts.cbParms); 
	returnValue += tmp;

	tmp.Format(L"DWORD  dwInterleaveEvery = %d\n", opts.dwInterleaveEvery);
	returnValue += tmp;
					...
					CString _compressor)
{
		...
		if(getFOURCC(_compressor) == NULL)
		{
			...
			MessageBox(NULL, dumpAVICOMPRESSOPTIONS(opts), L"AVICOMPRESSOPTIONS", MB_OK);
		}		
...
BOOL AVI_AddFrame(PAVISTREAM psCompressed, int time, LPBITMAPINFOHEADER lpbi)
{
	...
	{
		CString strMsg;
		strMsg.Format(L"Error: AVIStreamWrite, error %d",hr);
